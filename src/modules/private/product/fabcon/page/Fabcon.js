...
const Fabcon = () => {
  ...
  const handleUpdate = (values) => {
    Http.get(`update/fabcons/${values}`).then(
    );
  };
          ...
          <TableBody>
            {fabconData.map((fabcon) => (
              <TableRow key={fabcon.id}>
                ...
                <TableCell size="small" sx={{width:"40%"}}>{fabcon.fabcon_price}</TableCell>
...
<TableCell size="small">
  {fabcon && fabcon.image && (
    <img src={fabcon.image_url} alt="Fabcon Image" style={{ width: '100px' }} />
  )}
</TableCell>
           ...
           {selectedItem && (
              <ShowFabcon sx={{maxWidth:"500px"}} selectedItem={selectedItem} onClose={() => setSelectedItem(null) } />
           )}   
    </>
  );
};
